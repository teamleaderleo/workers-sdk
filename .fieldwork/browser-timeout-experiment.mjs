...
const BINDING_PATH = "packages/miniflare/src/workers/browser-rendering/binding.worker.ts";
...
function replaceInTest(source, testName, from, to, label) {
	...
	const replaced = replaceOnce(testSource, from, to, label);
	return source.slice(0, testStart) + replaced + source.slice(testEnd);
}
...
async function applyLaunchFaultInjection() {
...
async function applyConnectFaultInjection() {
	let source = await readFile(BINDING_PATH, "utf8");
	source = replaceOnce(
		source,
		`\t\t\treturn await fetch(url, { ...init, signal });`,
...
switch (command) {
	case "test-policy":
		...
		break;
	case "launch-fault":
		await applyLaunchFaultInjection();
		break;
	case "connect-deadline":
		await applyConnectDeadline();
		break;
	case "connect-fault":
		await applyConnectFaultInjection();
		break;
	default:
		throw new Error("Usage: browser-timeout-experiment.mjs <test-policy|launch-fault|connect-deadline|connect-fault>");
}
