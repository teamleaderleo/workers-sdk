...
async function addCallerAbortProbe() {
  let spec = await readFile(SPEC, "utf8");
  spec = replaceOnce(spec,
...
`\t\tif (request.url.endsWith(\"session-abort\")) {\n\t\t\tconst controller = new AbortController();\n\t\t\tsetTimeout(() => controller.abort(new Error(\"Injected user-worker caller cancellation\")), 250);\n\t\t\ttry {\n\t\t\t\tconst response = await env.MYBROWSER.fetch(\"https://localhost/v1/acquire\", { signal: controller.signal });\n\t\t\t\treturn new Response(await response.text());\n\t\t\t} catch (e) {\n\t\t\t\treturn new Response(\`FIELDWORK_CALLER_ABORT \${e?.name ?? \"Error\"}: \${e?.message ?? String(e)}\`);\n\t\t\t}\n\t\t}\n\t\tif (request.url.endsWith(\"session\")) {\n\t\t\tconst newBrowserSession = await env.MYBROWSER.fetch(\"https://localhost/v1/acquire\")\n\t\t\treturn new Response(await newBrowserSession.text())\n\t\t}`,
"test worker session route");

  const marker = `\n\ttest(\n\t\t\"fieldwork caller abort probe\",\n\t\t{ timeout: 20_000 },\n\t\tasync ({ expect }) => {\n\t\t\tconst mf = new Miniflare({\n\t\t\t\tworkers: [{ config: { type: \"worker\", name: \"worker\", compatibilityDate: \"2024-11-20\", manifest: singleModuleManifest(BROWSER_WORKER_SCRIPT()), env: { MYBROWSER: { type: \"browser\" } } } }],\n\t\t\t});\n\t\t\tuseDispose(mf);\n\t\t\tconst started = Date.now();\n\t\t\tconst res = await mf.dispatchFetch(\"https://localhost/session-abort\");\n\t\t\tconst text = await res.text();\n\t\t\tconst elapsed = Date.now() - started;\n\t\t\tconsole.log(\"FIELDWORK_CALLER_RESPONSE\", elapsed, text);\n\t\t\tawait new Promise((resolve) => setTimeout(resolve, 750));\n\t\t\texpect(text).toContain(\"FIELDWORK_CALLER_ABORT\");\n\t\t\texpect(elapsed).toBeLessThan(2_000);\n\t\t}\n\t);\n`;
  const idx = spec.lastIndexOf("\n});");
  if (idx === -1) throw new Error("Unable to find browser describe close");
  spec = spec.slice(0, idx) + marker + spec.slice(idx);
...
switch (process.argv[2]) {
  ...
  case "caller-test": await addCallerAbortProbe(); break;
  default: throw new Error("usage: v3 <candidate|stall|nonretryable|caller-fault|cleanup-log|stall-test|nonretryable-test|caller-focused-test|caller-test>");
}
